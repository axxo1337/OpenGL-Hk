...
#include <iostream>
void ImGui::AlignedText(const char* text, ImGuiAligned alignment, bool y_centered)
{
    ImVec2 windowSize = ImGui::GetWindowSize();
    ImVec2 textSize = ImGui::CalcTextSize(text);
    ImGui::SetCursorPosX((windowSize.x - textSize.x) * ((float)alignment/10));

    if (y_centered)
        ImGui::SetCursorPosY((windowSize.y - textSize.y) * 0.5);

    ImGui::Text(text);
}

void ImGui::CustomText(const char* text, ImFont* font, ImGuiAligned alignment, bool y_centered)
{
    ImGui::PushFont(font ? font : ImGui::GetDefaultFont());
    ImVec2 windowSize = ImGui::GetWindowSize();
    ImVec2 textSize = ImGui::CalcTextSize(text);
    ImGui::SetCursorPosX((windowSize.x - textSize.x) * ((float)alignment / 10));
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, ImGuiAligned alignment)
{
    ImVec2 windowSize = ImGui::GetWindowSize();
    ImGui::SetCursorPosX((windowSize.x - size_arg.x) * ((float)alignment/10));
