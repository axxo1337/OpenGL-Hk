...
namespace ImGui
{
    ...
    // Widgets: CUSTOM TOOPIE Text
    IMGUI_API void          AlignedText(const char* text, ImGuiAligned alignment = 0, bool y_centered = false);
    IMGUI_API void          CustomText(const char* text, ImFont* font, ImGuiAligned alignment = 0, bool y_centered = false);

    // Widgets: Main
    // - Most widgets return true when the value has been changed or when pressed/selected
    // - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to query widget state.
    IMGUI_API bool          Button(const char* label, const ImVec2& size = ImVec2(0, 0), ImGuiAligned alignment = 0);   // button
...
enum ImGuiWindowFlags_
{
    ...
    // [CUSTOM TOOPIE]
    ImGuiWindowFlags_ControlPos             = 1 << 29,  // Control main window position (GLFW Only)
    ImGuiWindowFlags_Bottom                 = 1 << 30,
    ImGuiWindowFlags_Centered               = 1 << 21,
...
enum ImGuiAligned_
{
    ImGuiAlignedFlags_Left   = 0,
    ImGuiAlignedFlags_Middle = 5,
    ImGuiAlignedFlags_Right  = 10,
    // Each "Flag" follow the logic of, `VALUE/10` since floating points are not supported in enums
};
